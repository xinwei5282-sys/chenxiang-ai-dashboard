...
默认自动运行快慢分层的模拟数据：浏览量2–5秒、AI服务4–8秒、流通记录8–15秒、加工15–25秒、档案20–40秒、批次30–50秒递增。档案总数从三款商品汇总，并同步中央节点；基地和链路数量固定。数字以650ms过渡，新业务记录进入滚动列表。后台暂停增长，刷新页面从初始数据重新开始。所有变化均为前端模拟，未连接真实业务接口。

`npm test`包含模拟事件与8小时数值检查；`npm run verify:simulation`检查真实页面增长、合计一致性、记录更新与减动效显示。

## 实时预警滚动（2026-09-18）

实时预警以20条样例初始化，并陆续更新模拟业务记录；同屏可见5行，约每4秒向上移动一行，80秒无缝循环。状态保留预警、提示、正常三类；系统开启“减少动态效果”时静止显示首5行。此效果仅为前端展示，不接实时告警接口。

聚焦验证：`npm run verify:alerts`，覆盖实际滚动、末尾衔接、文字边界和减少动态效果。
...
顶部四项指标；左侧AI能力、实时预警样例和品牌故事；中央六环节与沉香树球；右侧数字人插画和产品TOP3；底部四项展示内容。所有主内容同时可见，无路由切换。中心组合图与山林底图是参考生成的独立素材；10个图标以原稿图集独立定位；数字人复用原稿插画区域。标题、指标、六节点文字、左右列表和底部入口为HTML。右下角产品 TOP3 按顺序使用用户提供的马到福来、祥云雅韵A、心灯长明熏香套装原图，统一66×55px缩略图并等比居中裁切铺满；档案和浏览数由模拟运行持续递增。Bootstrap Icons以本地SVG文件使用。
